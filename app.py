...
if st.button("Predict type of obesity"):
    ...
    if result=='Normal_Weight':
        st.success(f'The obesity type is {result}')
        #st.write(f'The obesity type is {result}')
        st.markdown("""#### <p style="color:orange">* BMI between 18.5 and 24.9 ,Generally considered to have a lower risk of health problems compared to other BMI categories. However, even within the "normal weight" BMI range, individuals who have a high percentage of body fat or poor lifestyle habits may still be at risk for certain health issues.</p>"""unsafe_allow_html=True)
    ...
    if result=='Overweight_Level_I':
        ...
        st.markdown("""#### <p style="color:orange">* BMI between 25.0 and 29.9 ,Increased risk for health problems such as high blood pressure, high cholesterol, heart disease, stroke, type 2 diabetes, sleep apnea, and certain cancers.</p>"""unsafe_allow_html=True)
        st.markdown("""#### <p style="color:white">* Aim to achieve a modest weight loss of 5-10% of your body weight through a combination of a healthy diet and increased physical activity. Adopting healthy habits such as regular exercise, reducing calorie intake, avoiding processed and high-fat foods, and increasing intake of fruits, vegetables, and whole grains can help reduce the risk of health problems associated with excess weight.</p>"""unsafe_allow_html=True)
    ...
    if result=='Obesity_Type_I':
        st.success(f'The obesity type is {result}')
        #st.write(f'The obesity type is {result}')
         st.markdown("""#### <p style="color:orange">*BMI between 35.0 and 39.9 ,Increased risk for the same health problems as those in Overweight Level II, as well as other health problems such as gallbladder disease, osteoarthritis, and infertility.</p>"""unsafe_allow_html=True)
    ...
    if result=='Obesity_Type_II':
        st.success(f'The obesity type is {result}')
        #st.write(f'The obesity type is {result}')
        st.markdown("""#### <p style="color:orange">*BMI between 40.0 and 44.9, Increased risk for the same health problems as those in Obesity Type I, but at a higher risk due to the increased amount of excess body fat.</p>"""unsafe_allow_html=True)
    ...
    if result=='Obesity_Type_III':
        st.success(f'The obesity type is {result}')
        #st.write(f'The obesity type is {result}')
        st.markdown("""#### <p style="color:orange">*BMI 45 or above, Increased risk for the same health problems as those in Obesity Type II, but at a higher risk due to the significantly higher amount of excess body fat.</p>"""unsafe_allow_html=True)
    ...
    if result=='Insufficient_Weight':
        ...
        st.markdown("""#### <p style="color:orange">*BMI below 18.5,  Increased risk for malnutrition, weakened immune system, anemia, osteoporosis, and other health problems related to being underweight.</p>"""unsafe_allow_html=True)
        st.markdown("""#### <p style="color:white">*It is important to consult with a healthcare professional or registered dietitian to determine the underlying cause of insufficient weight and develop a personalized plan to achieve a healthy weight. This may include increasing calorie and nutrient intake through a balanced diet, incorporating strength training to build muscle mass, and addressing any underlying medical conditions.</p>"""unsafe_allow_html=True)
